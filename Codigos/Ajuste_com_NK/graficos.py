...
pars_otimo = pars.copy()

for i, key in enumerate(keys_pars):
    pars_otimo[key] = array_otimo[i]
...
pars_otimo['B0'] = 2.5e5
pars_otimo['NK0'] = 2.0e5

t_eval = np.linspace(0, 37, 3500)
sol = solve_ivp(modelo, [0.0, 37.0], y0, args = (pars_otimo,), method= 'Radau', t_eval = t_eval)
...
ax[2, 0].scatter(nk['x'], np.log10(nk['y']), label = 'Dados', color = 'red')
