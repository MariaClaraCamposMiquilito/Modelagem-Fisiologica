...
if not sol_base.success:
    raise RuntimeError("Erro na solução base: " + sol_base.message)
...
def calcula_sensibilidade(delta):
    ...
    # shape[0] retorna o tamanho da primeira dimensão do vetor = quantas equações tem 
    ...
    for i, name in enumerate(params_keys):
        ...
        y0_plus = y0.copy()
        y0_plus[0] = pars_plus['V0']

        y0_minus = y0.copy()
        y0_minus[0] = pars_minus['V0']
        ...
        if not sol_plus.success or not sol_minus.success:
...
for nome_var, idx_var in variaveis.items():
    ...
    fig.savefig(
        os.path.join(path, f"analise_local_{nome_var}.png"),
        dpi=300,
        bbox_inches='tight'
    )
